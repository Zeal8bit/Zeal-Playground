...
(() => {
  ...
  Parser.prototype = {
    ...
    isString: function () {
      ...
      if (
        (this.pos < this.expression.length && this.expression.charAt(this.pos) == "'") ||
  ...
  var parseLine = (s, macros, opts = { stopFlag: null, olds: null, assembler: null }) => {
    ...
    if (
      ...
      s.opcode === '.PRAGMA' ||
      s.opcode === 'END' ||
      s.opcode === '.END' || //6809 assembler ops
      s.opcode === 'BSZ' ||
      s.opcode === 'FCB' ||
      s.opcode === 'FCC' ||
      s.opcode === 'FDB' ||
      ...
      s.opcode === 'RMB' ||
      s.opcode === 'ZMB' ||
      s.opcode === 'SETDP' || //65816
      s.opcode === '.M8' ||
      s.opcode === '.X8' ||
      s.opcode === '.M16' ||
      s.opcode === '.X16' || //phase, dephase
      s.opcode === '.PHASE' ||
      s.opcode === '.DEPHASE' ||
      s.opcode === '.SETPHASE' ||
      s.opcode === 'ALIGN' ||
      s.opcode === '.CSTR' ||
      s.opcode === '.ISTR' ||
      s.opcode === '.PSTR' || //segments
      s.opcode === '.CSEG' ||
      s.opcode === '.DSEG' ||
      s.opcode === '.ESEG' ||
      s.opcode === '.BSSEG' || //modules
      s.opcode === '.EXPORT' ||
      s.opcode === '.EXTERN' ||
      s.opcode === 'DB' ||
      s.opcode === 'DW' ||
  ...
  var prepro = async (V, opts = {}, fullfile) => {
    ...
    for (const item of V) {
      ...
      if (opcode === '.MACRO') {
        ...
        if (macroName.endsWith(':')) macroName = macroName.slice(0, -1);
  ...
  var unroll = (V, macros, uniqseed, opts) => {
    ...
    for (const [i, s] of V.entries()) {
      ...
      for (const [j, macroItem] of m.entries()) {
        if (j === 0) continue;
        const preline = macroParams(macroItem, s.params, i + uniqseed, pars, s.numline);
        preline.bytes = 0;
